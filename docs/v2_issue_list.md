...
| # | Location | Defect |
|---|----------|--------|
| P1-1 | core `events.R` | No validity mask, no seconds-based minimum bout, no maximum merge gap, no valid analyzed time, no median bout, no percent-valid-time. |
...
| P1-5 | `IsInZone()`, `ZoneReport()` | `sp::point.in.polygon(...) == 1` excludes boundary points, so a point on a shared zone edge belongs to no zone. Inconsistent with core `points_in_polygon()`, which includes the boundary. |

## P1 - tests and CI

| # | Defect |
|---|--------|
| P1-6 | No CI configuration of any kind. |
