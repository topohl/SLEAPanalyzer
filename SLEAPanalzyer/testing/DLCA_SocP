...
for (file in file_list) {
  ...
  # Count transitions
  threshold <- 10 # number of frames "false" in between transitions
  IsInZoneLeft <- GetDistances(Tracking, "socl", "nose") < 6
  IsInZoneRight <- GetDistances(Tracking, "socr", "nose") < 6
  freq_ContactLeft <- sum(IsInZoneLeft) / total_time
  freq_ContactRight <- sum(IsInZoneRight) / total_time
  trans_ContactLeft <- sum(diff(c(FALSE, IsInZoneLeft)) == c(1, -1) & rollapply(IsInZoneLeft, 2*threshold+1, sum, align="center", na.pad=TRUE) >= threshold)
  trans_ContactRight <- sum(diff(c(FALSE, IsInZoneRight)) == c(1, -1) & rollapply(IsInZoneRight, 2*threshold+1, sum, align="center", na.pad=TRUE) >= threshold)

  # Create a data frame with the time_Left, time_Right, time_nose_Left, and time_nose_Right values
  df <- data.frame(ContactLeft = ContactLeft, ContactRight = ContactRight, ProxLeft = ProxLeft, ProxRight = ProxRight, 
                 freq_ContactLeft = freq_ContactLeft, freq_ContactRight = freq_ContactRight,
                 trans_ContactLeft = trans_ContactLeft, trans_ContactRight = transContactRight)
  ...
  print(paste("Processed file", input_file))
}
